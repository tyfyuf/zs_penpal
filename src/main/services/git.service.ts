...
async function ensureCommitIdentity(git: SimpleGit): Promise<void> {
  ...
  if (!name && !existingName) await git.addConfig('user.name', 'Penpal', false, 'local')
  if (!email && !existingEmail) await git.addConfig('user.email', 'penpal@localhost', false, 'local')
}
