...
> [!IMPORTANT]
> **证据更新（2026-08-17）**：本报告最初撰写时尚未取得真实配置、用量与失败样本，其中“上下文配置/输入超窗”为待验证假设。后续核验已确认本次不是样本输入超过 100 万 token 上下文；DeepSeek V4 默认思考是当前样本的高置信触发因素，但更上层的架构根因是 **缺少端点/模型能力协商，固定 completion 预算，输出协议膨胀，且重试不自适应**。修复必须面向用户自由接入的各类 OpenAI-compatible 模型，而不是只写 DeepSeek 特例。请以 `docs/summary-distillation-failure-root-cause-and-solution-2026-08-17.md` 的结论为准。

**报告日期：2026 年 8 月 17 日**
**原始范围：** 项目内已实现代码、`PROJECT_CONTEXT.md`、三层记忆定稿文档与当时 Git 历史；原稿不包含真实配置、运行日志、失败样本或联网 API 核验。
**后续补证：** 已取得用户实际配置、2026-08-17 用量聚合、日志目录、两份失败样本，并核对 DeepSeek 官方 API 文档；详见新的根因与方案报告。

---

## 1. 接手结论

当前最近一次提交为 `eecf1d6`（保留本现状报告）；工作区包含本轮报告/交接文档更新和用户提供的失败样本，摘要生成实现尚未修改。`npm run typecheck` 已在本轮重新通过。摘要、资源蒸馏、聊天小摘要、大摘要（rollup）及注入/记忆规划均已有实现，主实现集中在 `src/main/services/summary.service.ts`。
...
1. **“输入超过上下文窗口”已被本次样本排除。** 用户配置为 1,000,000 tokens；稳定失败的“硝烟粉笔灰”仅 3,215 字符，远低于应用 60% 输入门槛。
2. **通用模型适配问题已确认。** 当前只有 `baseURL + model + contextLimit`，没有记录结构化输出、reasoning 控制、输出 token 参数、采样参数和 usage 形态等能力。DeepSeek V4 默认 thinking 是当前配置下的具体触发因素；其他 OpenAI-compatible 端点会以“不支持参数、结构化能力不同、usage 缺失或弱模型语义失败”等方式暴露同一缺陷。
3. **直接失败点已确认，单次内部明细仍缺失。** 当前错误只会由 `finish_reason=length`，或可见内容为空/无法形成非空摘要触发；由于手动路径未记录逐次 finish reason、reasoning token 和可见内容长度，无法对历史两次请求做逐 token 取证。
4. **超长文档单次全文提取仍是独立架构风险。** “龙常剧情书”约 11 万字，本次成功但历史高频失败，说明单次 8,192-token JSON 闭合不稳定，后续仍需分级提取。
5. **全局串行队列是体验放大器，不是本次核心逻辑根因。** 它可能造成排队和超时感，但不能解释短文档稳定进入“不完整/空或截断”错误分支。
...
本轮已取得：实际 API Base URL、模型名、1,000,000-token 配置、两份样本、完整错误文案、2026-08-17 用量聚合、日志目录状态，以及失败后旧文件/假成功的历史现象。证据已足以进入方案讨论，不需要继续等待同类样本。

下一轮建议只确认以下产品/工程决策：

1. 是否接受统一 structured-task 执行器与端点/模型 capability profile；
2. 是否采用“内置 preset + 低成本探测 + 运行时降级 + 高级手动覆盖”的兼容策略；
3. 结构化任务默认最小化 reasoning，由 provider adapter 翻译，主聊天策略独立；
4. 长文档是否直接进入分级提取，还是先单次尝试、失败后降级；
5. 可接受的调用次数、等待时间、进度和取消交互；
6. 摘要目标是“可注入的紧凑记忆”还是“尽可能完整的剧情档案”；
7. 是否先实施 P0 通用兼容层、可观测性与严格校验，再实施 P1 长文档流程。

当前最不建议的做法仍是只给 DeepSeek 加私有字段、只增加提示词限制或盲目提高 `max_tokens`；这无法修复多端点能力差异、无限输出协议、同参数重试和弱校验组成的通用故障链。
